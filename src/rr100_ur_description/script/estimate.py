...
class WorkspaceSampler:
    def __init__(
        self,
        frame,
        collision_service: str,
        kinematics_topic: str,
        n_points: Union[SupportsIndex, Sequence[SupportsIndex]],
        xrange: list,
        yrange: list,
        zrange: list,
        angle_cone: float = np.pi / 4,
        prefix: str = "",
        connect: bool = True,
    ):
        self.prefix = prefix
        ...
        self.targets = [
            ptf.translation_matrix(point) @ ptf.euler_matrix(np.pi, 0, 0)
            for point in self.sample
        ]
        self.target_poses = [
            WorkspaceSampler.matrix_to_pose(target, self.frame)
            for target in self.targets
        ]

        if connect:
            self.kinematics_client = SimpleActionClient(kinematics_topic, URGoToAction)
            self.collisions_checker = rospy.ServiceProxy(
                collision_service, CheckCollision
            )
            ...
            self.joint_states_pub = rospy.Publisher(
                "/joint_states", JointState, queue_size=10, latch=True
            )
            self.joint_names = [
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--topic",
        type=str,
        default="/kinematics_server/goal_pose",
        help="Kinematics server action topic",
    )
    parser.add_argument(
        "-c",
        "--collision-service",
        type=str,
        default="/kinematics_server/check_collision",
        help="Kinematics server collision checking service",
    )
    parser.add_argument(
        "-f",
        "--frameid",
        ...
        help="Frame ID to work in",
    )
    parser.add_argument(
        "-o", "--output", type=str, help="Output file for sampled points"
    )
    parser.add_argument(
        "-n", "--npoints", type=int, default=200, help="Number of points to generate"
    )
    parser.add_argument(
        "-x",
        "--xrange",
        ...
        nargs=2,
        default=[0.0, 1.0],
        help="Range of values along the X axis for sampled points",
    )
    parser.add_argument(
        "-y",
        "--yrange",
        ...
        nargs=2,
        default=[0.0, 1.0],
        help="Range of values along the Y axis for sampled points",
    )
    parser.add_argument(
        "-z",
        "--zrange",
        ...
        nargs=2,
        default=[0.0, 1.0],
        help="Range of values along the Z axis for sampled points",
    )
